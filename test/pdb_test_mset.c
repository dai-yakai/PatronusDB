...
void testcase_mset_100w(int connfd) {
    ...
    int batches = global_batch;           
    int keys_per_batch = global_batch_num; 
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        printf("Usage: %s <server_ip> <server_port> <batches>\n", argv[0]);
        return -1;
    }
    char* ip = argv[1];
    ...
    global_batch = batches;
    global_batch_num = 1000000 / batches;
    
    return pdb_testcase_mset(ip, port);
}